...
console.log(splitMe("ciao mondo tutto bene"));
...
function onlyLetters(x){
  ...
  return x.split("").filter(val => !/[0-9]/.test(val)).join("");
...
console.log(isThisAnEmail("leo.giardinogmail.com"));
...
function howManyDays(x){
  let data1 = new Date(x);
  let data2 = new Date();
  let diffTempo = data2 - data1;
  let giorniDiff = Math.floor(diffTempo / (1000 * 60 * 60 * 24));

  return giorniDiff;

 
}
console.log(howManyDays("1994-12-23"));
...
console.log(isTodayMyBirthday("11-14"));
...
function onlyTheYears(){
  ...
  let years = movies.filter(film => film.Type ==="movie").map(film => film.Year);
  return years;
  }
...
function searchByTitle(titolo){
  ...
  let film = movies.filter(val => val.Title === titolo);
  return film;
...
function searchAndDivide(stringa){
  ...
  let film = movies.filter(val => val.Title.includes(stringa) ? match.push(val) : unmatch.push(val));
  arry.match = match;
  arry.unmatch = unmatch;
...
function redA(){
  ...
  a.forEach(ele => ele.style.backgroundColor = "red")
...
function tree(x) {
  for (let i = 1; i <= x; i++) {
    ...
    // Aggiungi spazi vuoti prima degli asterischi
    for (let y = x; y > i; y--) {
      riga += " ";
    }
